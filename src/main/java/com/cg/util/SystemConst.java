package com.cg.util;

public class SystemConst {
    public static final Integer FLAG_0 = 0;
    public static final Integer FLAG_1 = 1;
    public static final Integer OK = 200;
    public static final Integer ERROR = 201;
    public static final String SUCCESS = "操作成功";
    public static final String USERNAME_NOT_EMPTY = "用户名不能为空";
    public static final String PASSWORD_NOT_EMPTY = "密码不能为空";
    public static final String NEW_PASSWORD_NOT_EMPTY = "新密码不能为空";
    public static final String USERNAME_OR_PASSWORD_ERROR = "用户名或密码错误";
    public static final String USER_EXIST = "用户已存在";
    public static final String USER_NOT_EXIST = "用户不存在";
    public static final String LOGIN_KEY = "login:";
    public static final String NO_LOGIN = "未登录";
    public static final String NO_AUTHORITY = "未授权";
    public static final String AUTHORITY_ERROR = "认证失败";
    public static final String PARAMETER_ERROR = "传递参数错误";
    public static final String INTEGER_TRANSFORM_ERROR = "整型转换错误";
    public static final String USER_FORBIDDEN = "用户被禁用";
    public static final String FILE_NOT_EMPTY = "文件不能为空";
    public static final String FILE_TYPE_ERROR = "文件类型必须为png,jpg,jpeg";
    public static final String QI_NIU_ERROR = "七牛云上传失败";
    public static final String PHONE_REGEX = "^1[3-9]\\d{9}$";
    public static final String EMAIL_REGEX = "[a-zA-Z0-9]+@[a-zA-Z0-9]+\\.[a-zA-Z0-9]+";
    public static final String PHONE_ERROR = "电话号码不合法";
    public static final String EMAIL_ERROR = "邮箱不合法";
    public static final String GENDER_ERROR = "性别不合法";
    public static final String STATUS_ERROR = "状态不合法";
    public static final String FORBIDDEN_FAIL = "禁用或启用失败";
    public static final String EXIT_FAIL = "禁用或启用失败";
    public static final String PASSWORD_NOT_CONSISTENT = "密码不一致";
    public static final String PASSWORD_ERROR = "密码错误";
    public static final String TITLE_NOT_EMPTY = "标题不能为空";
    public static final String TITLE_EXIST = "标题已存在";
    public static final String SURVEY_NOT_EXIST = "问卷不存在";
    public static final String EXPIRE_TIME_NOT_EMPTY = "截止时间不能为空";
    public static final String EXPIRE_TIME_ERROR = "截止时间小于当前时间";
    public static final String TYPE_ERROR = "类型不合法";
    public static final String REQUIRED_ERROR = "必答字段不合法";
    public static final String QUESTION_NOT_EXIST = "问题不存在";
    public static final String SORT_ERROR = "顺序字段不合法";
    public static final String OPTION_NOT_EXIST = "选项不存在";
    public static final String OPTION_EXIST = "选项已存在";
    public static final String CONTENT_NOT_EMPTY = "内容不能为空";
    public static final String NULL_POINTER = "空指针异常";
    public static final String ANSWER_NOT_EMPTY = "答案不能为空";
    public static final String LINK_NOT_EXIST = "链接不存在";
    public static final String IP_EXIST = "用户已填写问卷";
    public static final String SURVEY_NOT_PUBLISH = "问卷还未发布";
    public static final String SURVEY_PUBLISH = "问卷已发布";

    public static final String SAVE_FAIL = "保存数据失败";
    public static final String UPDATE_FAIL = "更新数据失败";
    public static final String DELETE_FAIL = "删除数据失败";


}
